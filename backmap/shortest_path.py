...
def run_min_distance(new_elements, old_elements):
    matched, unmatched = [], []
    new_element_index = feature_sort(new_elements, key=lambda x: x.startLocation)
    old_element_index = feature_sort(old_elementsld, key=lambda x: x.startLocation)

    for i in range(len(old_elements_index)):  # len index dependent on num chr
        m, um = match_chromosome_elements(new_element_index[i], old_elements_index[i])
        matched.append(m)
        unmatched.append(um)
...
def match_chromosome_elements(new_elements, old_elements):
    ...
    '''
    statuses, matches, unmatches = get_statuses(new_elements), [], []  # should be same as old
    for s in statuses:
...
def minimize_distance(list_one, list_two):
    ...
    for i in range(0, d+1):
        s = sum(abs(x.startLocation - y.startLocation) for x, y in zip(shorter_list, longer_list[i:len(shorter_list)+i]))
        if s < cs[0]:
